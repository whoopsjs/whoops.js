...
function getAssignedValue(node) {
  if (node.type === 'CallExpression') {
    ...
    return getAssignedValue(node.left) + node.right.operator + getAssignedValue(node.right);
  } else {
    return [];
  }
}
...
function dangerousNode(node) {
  ...
  else if (node.callee.property && (node.callee.property.name === 'createElement' || node.callee.property.name === 'getElementsByTagName') && contains(monitoredTags, node.arguments[0].value)) {
    return true;
  }
  //Also Dangerous to create a textNode of a dangerous Value
  else if (node.callee.property && (node.callee.property.name === 'createTextNode' || node.callee.property.name === 'createElement' || node.callee.property.name === 'createAttribute')) {
  ...
  else if (node.callee.property && node.callee.property.name === 'getElementsByName') {
  ...
  else if (node.callee.property && node.callee.property.name === 'getElementByID') {
    for (var i = 0; i < inputs.length; i++) {
...
module.exports = function (tree) {
  ...
  walk.recursive(tree.data.cfg, {}, {
    ...
    VariableDeclaration: function (node, state, c) {
      ...
      for (var i = 0; i < declarator.length; i++) {
        ...
        if (subnode.type === 'CallExpression' && dangerousNode(subnode)) {
      ...
      if (node.operator === '=') {
        ...
        if (containsName(inputs, name)) {
          ...
          if (node.right.type === 'Literal') {
            if (node.left.property !== undefined && node.left.property.name === 'name') {
              for (var i = 0; i < inputs.length; i++) {
                if (inputs[i][0] === name) {
                  inputs[i][2] = node.right.value;
                }
              }
            } else if (node.left.property !== undefined && node.left.property.name === 'id') {
              for (var i = 0; i < inputs.length; i++) {
                if (inputs[i][0] === name) {
        ...
        else if (node.right.type == 'Identifier' && containsName(inputs, node.right.value)) {
  ...
  walk.recursive(tree.data.cfg, {}, {
    FunctionDeclaration: function (node, state, c) {
      for (var i = 0; i < assignments.length; i++) {
        if (assignments[i][3] >= node.start && assignments[i][4] <= node.end && assignments[i][1]) {
  ...
  walk.recursive(tree.data.cfg, {}, {
    CallExpression: function (node, state, c) {
      ...
      if ((node.callee.property !== undefined && node.callee.property.name === 'appendChild') ||
        ...
        var subNode = node;
        while (subNode.arguments[0] !== undefined && subNode.arguments[0].type === 'CallExpression' && !(subNode.callee.object !== undefined && subNode.callee.object.name === 'document')) {
        ...
        if (objectNode.name === 'document' || containsName(inputs, subNode.arguments[0].name)) {
          var name;
          ...
          //If value to be appanded come from a function, then take the function name.
            ...
            if (subNode.arguments[0].callee.name !== undefined) {
              ...
              name = nameNode.callee.object.name + '.' + nameNode.callee.property.name + '("' + nameNode.arguments[0].value + '")';
            }
          }
          ...
          if (nowDangerous || contains(evilFunctions, name) || (subNode.arguments[0].type === 'CallExpression') && dangerousNode(subNode.arguments[0])) {
